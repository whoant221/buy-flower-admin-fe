import React from 'react'
import { Link } from 'react-router-dom'

export default function SideBar() {
    return (
        <aside class="app-sidebar">

            <div class="app-sidebar__user">
                <img class="app-sidebar__user-avatar" src="data:image/png;base64," width="50px"
                    alt="LOGO"></img>
                <div>
                    <p class="app-sidebar__user-name"><b>Admin</b></p>

                </div>
            </div>
            <hr />
            <ul class="app-menu">
                <li>
                    <Link class="app-menu__item" to="/">
                        <i class='app-menu__icon bx bx-tachometer'></i>
                        <span
                            class="app-menu__label">Thống kê
                        </span>
                    </Link>
                </li>
                <li><Link class="app-menu__item" to="/customer"><i class='app-menu__icon bx bxs-user'></i>
                    <span class="app-menu__label">Quản lý khách hàng</span></Link></li>
                <li><Link class="app-menu__item" to="/product"><i
                    class='app-menu__icon bx bx-purchase-tag-alt'></i><span class="app-menu__label">Quản lý sản phẩm</span></Link>
                </li>
                <li><Link class="app-menu__item" to="/order"><i class='app-menu__icon bx bx-task'></i><span
                    class="app-menu__label">Quản lý đơn hàng</span></Link></li>
                <li><Link class="app-menu__item" to="/voucher"><i
                    class='app-menu__icon bx bx-pie-chart-alt-2'></i><span class="app-menu__label">Voucher</span></Link>
                </li>
                {/* <li><Link class="app-menu__item" to="/turnover"><i
                    class='app-menu__icon bx bx-pie-chart-alt-2'></i><span class="app-menu__label">Báo cáo doanh thu</span></Link>
                </li> */}
            </ul>
        </aside>
    )
}
